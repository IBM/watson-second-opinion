...
watsonAddDocument(reviews, credentials) {
  function addJsonDocument(file, retryAttempt) {
    discovery.addJsonDocument({
      ...
      file: file
        ...
        if (error) {
          ...
          const retryTime = Math.pow(2, retryAttempt)*100 - Math.random()*Math.pow(2, retryAttempt-1)*100;
          setTimeout(function () {
              addJsonDocument(file, retryAttempt+1); }, retryTime);
        }
    });
  }
  ...
  var promise = new Promise(function (resolve,reject) {
    ...
    function (next) {
        ...
        while(index<temporaryArray.length) {
          addJsonDocument(temporaryArray[index], 0);
          index++;
          uploadedReviews++;
